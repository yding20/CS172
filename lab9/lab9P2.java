...
public class lab9P2 {
    ...
    public Node reconstruct_tree(int[] inOrder, int[] preOrder) {
        ...
        reconstruct_tree(currentroot, inOrder, preOrder, 0, n);
        return currentroot;
    }

    private void reconstruct_tree(Node currentroot, int[] inOrder, int[] preOrder, int lo, int hi) {
        ...
        if (i > 0 ) {
            ...
            currentroot.left = new Node(preOrder[lo]);
            level_order_print();
            reconstruct_tree(currentroot.left, leftinOrderarray, leftpreOrderarray, 0, i-1);
        }
        ...
        if (rightlength > 0) {
            ...
            currentroot.right = new Node(preOrder[lo+i]);
            level_order_print();
            reconstruct_tree(currentroot.right, rightinOrderarray, rightpreOrderarray, 0, rightlength-1);
        }
    }
...
// Link to one solution which is more space efficient.
